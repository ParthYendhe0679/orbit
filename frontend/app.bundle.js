...
(() => {
  ...
  // src/utils/dom.ts
  function safeText(el3, val) {
    if (el3) el3.textContent = String(val);
  }
  ...
  function showAuthError(message) {
    const el3 = document.getElementById("login-error") || document.getElementById("signup-error");
    if (el3) {
  ...
  function switchToTab(tabName) {
    ...
    if (tabName === "dashboard") {
      ...
      if (typeof window.initStrategyLab === "function") window.initStrategyLab();
    }
  }
  ...
  function populateBotForm() {
    ...
    const setVal = (id, v) => {
      const el3 = getElement(id);
      if (el3 && v !== void 0 && v !== null) el3.value = String(v);
    };
  ...
  function setBotCapitalMax() {
    const el3 = getElement("bot-capital");
    if (el3 && _botAvailableBalance) el3.value = String(Math.floor(Number(_botAvailableBalance) * 100) / 100);
  }
  ...
  function showBotConfigError(message) {
    const el3 = getElement("bot-config-error");
    if (!el3) return;
    el3.textContent = message || "";
    el3.classList.toggle("hidden", !message);
  }
  ...
  function updateBotButtons() {
    ...
    ["bot-market", "bot-capital", "bot-target", "bot-maxloss", "bot-leverage", "bot-save-btn", "bot-max-btn"].forEach((id) => {
      const el3 = getElement(id);
      if (el3) el3.disabled = live;
    });
    document.querySelectorAll(".bot-asset-chip").forEach((b) => {
      b.disabled = live;
    });
  }
  function setBotBar(id, pct2) {
    const el3 = getElement(id);
    if (el3) el3.style.width = `${Math.max(0, Math.min(100, Number(pct2) || 0))}%`;
  }
  ...
  function renderBotActiveTrades(trades) {
    ...
    tbody.innerHTML = trades.map((pos) => {
      ...
      const pct2 = margin > 0 ? (pnl / margin * 100).toFixed(2) : "0.00";
      return `
            <tr>
                ...
                <td class="${pnl >= 0 ? "text-green" : "text-red"}"><strong>${botSigned(pnl)}</strong> <span style="font-size:11px;opacity:0.8;">(${pnl >= 0 ? "+" : ""}${pct2}%)</span></td>
  ...
  function handleMetrics(msg) {
    if (msg.consensus && msg.consensus.signal) {
      ...
      const el3 = getElement("overview-consensus-status");
      if (el3) {
        el3.textContent = sig;
        el3.className = sig === "BUY" || sig === "BULLISH" ? "text-green" : sig === "SELL" || sig === "BEARISH" ? "text-red" : sig === "MIXED" ? "text-yellow" : "text-blue";
      }
    }
    if (typeof msg.sentiment === "number") {
      const s = msg.sentiment;
      const el3 = getElement("overview-sentiment-status");
      if (el3) {
        el3.textContent = s > 0.15 ? "BULLISH" : s < -0.15 ? "BEARISH" : "NEUTRAL";
        el3.className = s > 0.15 ? "text-green" : s < -0.15 ? "text-red" : "text-cyan";
      }
    }
    if (msg.trend && typeof msg.trend.strength === "string") {
      const strength = msg.trend.strength;
      const el3 = getElement("overview-trend-status");
      if (el3) {
        el3.textContent = strength.toUpperCase();
        el3.className = strength.includes("uptrend") ? "text-green" : strength.includes("downtrend") ? "text-red" : "text-yellow";
      }
    }
  }
  ...
  async function quickCloseTrade(tradeId) {
    const posList = store.get("openTrades") || [];
  ...
  function renderDashboardSummary(summary) {
    ...
    const setPnl = (id, value) => {
      const el3 = getElement(id);
      if (!el3) return;
      el3.textContent = (value >= 0 ? "+" : "") + formatINR(value);
      el3.className = "stat-value " + (value >= 0 ? "text-green" : "text-red");
    };
  ...
  async function refreshBrainModal() {
    ...
    try {
      ...
      if (change) {
        const pct2 = num(mkt.price_change_pct);
        change.textContent = `${pct2 >= 0 ? "+" : ""}${pct2.toFixed(2)}%`;
        change.className = pct2 >= 0 ? "brain-sub-val text-green" : "brain-sub-val text-red";
      }
  ...
  // src/ui/strategyLabController.ts
  var supported = null;
  var currentStrategy = null;
  var currentUnderstanding = null;
  var lastResult = null;
  ...
  var el2 = getElement;
  function setBusy(id, busy, idleHtml, busyHtml) {
    const button = el2(id);
    if (!button) return;
  ...
  function money(value, currency = "$") {
    ...
    const sign = num3 < 0 ? "-" : "";
    return sign + currency + Math.abs(num3).toLocaleString(void 0, {
  ...
  function num2(value) {
    if (value === null || value === void 0 || value === "" || Number.isNaN(Number(value))) return "\u2014";
    return String(value);
  }
  function when(iso) {
    if (!iso) return "\u2014";
  ...
  function showStatus(message, kind = "info") {
    ...
    const icon = kind === "error" ? "fa-triangle-exclamation" : kind === "success" ? "fa-circle-check" : "fa-circle-info";
  ...
  async function initStrategyLab() {
    ...
    initialized = true;
    try {
      const res = await apiClient.get("/api/strategy-lab/supported");
      supported = unwrapData(res);
    } catch (err) {
      initialized = false;
      showStatus("Could not load the Strategy Lab: " + errorText3(err), "error");
      return;
    }
    const marketSelect = el2("slab-market");
    if (marketSelect) {
      marketSelect.innerHTML = supported.markets.map((m) => `<option value="${esc(m.value)}">${esc(m.label)}</option>`).join("");
    }
    const tfSelect = el2("slab-timeframe");
    if (tfSelect) {
      tfSelect.innerHTML = supported.timeframes.map((t) => `<option value="${esc(t.value)}"${t.value === "1d" ? " selected" : ""}>${esc(t.label)}</option>`).join("");
    }
    const periodSelect = el2("slab-period");
    if (periodSelect) {
      periodSelect.innerHTML = supported.periods.map((p) => `<option value="${esc(p.value)}"${p.value === "1y" ? " selected" : ""}>${esc(p.label)}</option>`).join("");
    }
    const capital = el2("slab-capital");
  ...
  function renderSupportedVocabulary() {
    const box = el2("slab-supported-list");
    if (!box || !supported) return;
    box.innerHTML = supported.vocabulary.indicators.map((i) => `<span class="slab-chip" title="${esc(i.label)}">${esc(i.name)}</span>`).join("");
  }
  function renderExamples() {
    const box = el2("slab-examples");
    if (!box || !supported) return;
    box.innerHTML = supported.examples.map((text) => `<button type="button" class="slab-example" data-example="${esc(text)}">${esc(text)}</button>`).join("");
    box.querySelectorAll(".slab-example").forEach((button) => {
      button.addEventListener("click", () => {
        const input = el2("slab-description");
        if (input) {
  ...
  function onStrategyLabMarketChange() {
    const market = el2("slab-market")?.value || "crypto";
    const option = supported?.markets.find((m) => m.value === market);
    const box = el2("slab-asset-examples");
    if (!box || !option) return;
    box.innerHTML = option.examples.map((s) => `<button type="button" class="slab-chip slab-chip-btn" data-symbol="${esc(s)}">${esc(s)}</button>`).join("");
    box.querySelectorAll(".slab-chip-btn").forEach((button) => {
      button.addEventListener("click", () => selectStrategyLabAsset(button.getAttribute("data-symbol") || ""));
    });
  }
  function onStrategyLabAssetInput(value) {
    if (searchTimer2) window.clearTimeout(searchTimer2);
    const query = (value || "").trim();
    const results = el2("slab-asset-results");
    if (!results) return;
    if (query.length < 2) {
      results.innerHTML = "";
    ...
    searchTimer2 = window.setTimeout(async () => {
      const market = el2("slab-market")?.value || "crypto";
      ...
      try {
        ...
        if (!rows.length) {
          ...
          results.innerHTML = rows.map((r) => `<button type="button" class="slab-asset-row" data-symbol="${esc(r.symbol)}">
                        <strong>${esc(r.symbol)}</strong>
                        <span>${esc(r.name || r.exchange || "")}</span>
                    </button>`).join("");
          results.querySelectorAll(".slab-asset-row").forEach((button) => {
            button.addEventListener("click", () => selectStrategyLabAsset(button.getAttribute("data-symbol") || ""));
          });
        }
  ...
  function selectStrategyLabAsset(symbol) {
    const input = el2("slab-asset");
  ...
  function onStrategyLabPeriodChange() {
    const custom = el2("slab-period")?.value === "custom";
    el2("slab-custom-range")?.classList.toggle("hidden", !custom);
  }
  async function generateStrategy() {
    if (interpreting) return;
    const description = el2("slab-description")?.value.trim() || "";
    if (!description) {
    ...
    el2("slab-understanding")?.classList.add("hidden");
    el2("slab-clarify")?.classList.add("hidden");
    try {
      const res = await apiClient.post("/api/strategy-lab/interpret", {
        description,
        symbol: el2("slab-asset")?.value || "",
        market: el2("slab-market")?.value || "",
        timeframe: el2("slab-timeframe")?.value || ""
      });
      ...
      showStatus(errorText3(err), "error");
    } finally {
      interpreting = false;
      setBusy(
        "slab-generate-btn",
  ...
  function renderUnderstanding(u) {
    const box = el2("slab-understanding");
    if (!box) return;
    const list = (items, icon) => items.length ? items.map((c) => `<li><i class="fa-solid ${icon}"></i> ${esc(c)}</li>`).join("") : `<li class="slab-muted">None</li>`;
    box.innerHTML = `
    ...
    box.classList.remove("hidden");
    el2("slab-run-btn")?.addEventListener("click", () => void runStrategyBacktest());
    el2("slab-edit-btn")?.addEventListener("click", openStrategyEditor);
    el2("slab-editor-cancel")?.addEventListener("click", () => el2("slab-editor")?.classList.add("hidden"));
    el2("slab-editor-apply")?.addEventListener("click", () => void applyStrategyEdit());
  }
  function renderClarification(data) {
    const box = el2("slab-clarify");
    if (!box) return;
    const questions = data.questions.length ? `<ul class="slab-question-list">${data.questions.map((q) => `<li>${esc(q)}</li>`).join("")}</ul>` : "";
    const unsupported = data.unsupported.length ? `<div class="slab-unsupported">
             <strong>Not supported yet:</strong>
             ${data.unsupported.map((u) => `<span class="slab-chip slab-chip-warn">${esc(u)}</span>`).join("")}
           </div>` : "";
    const suggestions = data.suggestions.length ? `<div class="slab-suggestions">
             <span class="slab-muted">Try one of these:</span>
             ${data.suggestions.map((s) => `<button type="button" class="slab-suggestion" data-text="${esc(s)}">${esc(s)}</button>`).join("")}
           </div>` : "";
    box.innerHTML = `
        ...
        ${suggestions}`;
    box.classList.remove("hidden");
    box.querySelectorAll(".slab-suggestion").forEach((button) => {
      button.addEventListener("click", () => {
        const input = el2("slab-description");
        if (!input) return;
  ...
  function openStrategyEditor() {
    const editor = el2("slab-editor");
    const textarea = el2("slab-editor-json");
    if (!editor || !textarea || !currentStrategy) return;
    textarea.value = JSON.stringify(currentStrategy, null, 2);
    editor.classList.remove("hidden");
  }
  async function applyStrategyEdit() {
    const textarea = el2("slab-editor-json");
    const issues = el2("slab-editor-issues");
    if (!textarea || !issues) return;
    let parsed;
    try {
      parsed = JSON.parse(textarea.value);
    } catch (err) {
      issues.innerHTML = `<strong>That is not valid JSON.</strong> ${esc(errorText3(err))}`;
      issues.classList.remove("hidden");
      return;
    }
    try {
      const res = await apiClient.post("/api/strategy-lab/validate", { strategy: parsed });
      const data = unwrapData(res);
      if (!data.valid) {
        issues.innerHTML = `<strong>${esc(data.message)}</strong><ul>${(data.issues || []).map((i) => `<li>${esc(i)}</li>`).join("")}</ul>`;
        issues.classList.remove("hidden");
        return;
      }
      currentStrategy = data.strategy;
      ...
      issues.innerHTML = `<strong>${esc(errorText3(err))}</strong>`;
      issues.classList.remove("hidden");
    }
  }
  async function runStrategyBacktest() {
    ...
    const symbol = el2("slab-asset")?.value.trim().toUpperCase() || "";
    if (!symbol) {
      showStatus("Choose an asset to backtest.", "error");
      return;
    }
    const period = el2("slab-period")?.value || "1y";
    const body = {
      strategy: currentStrategy,
      symbol,
      market: el2("slab-market")?.value || "crypto",
      timeframe: el2("slab-timeframe")?.value || "1d",
      period,
      initial_capital: Number(el2("slab-capital")?.value || 1e4),
      commission_percent: Number(el2("slab-commission")?.value || 0),
      description: el2("slab-description")?.value.trim() || ""
    };
    if (period === "custom") {
      body.start_date = el2("slab-start-date")?.value || "";
      body.end_date = el2("slab-end-date")?.value || "";
    ...
    backtesting = true;
    showStatus("");
    const runButton = el2("slab-run-btn");
    if (runButton) {
      runButton.disabled = true;
      runButton.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> Running backtest\u2026`;
    }
    el2("slab-results")?.classList.add("hidden");
    try {
      const res = await apiClient.post("/api/strategy-lab/backtest", body);
  ...
  function renderResults(result) {
    ...
    const tradeRows = trades.length ? trades.slice(0, 200).map((t) => `
            ...
            </tr>`).join("") : `<tr><td colspan="9" class="slab-muted slab-center">
    ...
    box.innerHTML = `
        <div class="slab-results-head">
            <div>
                ...
                <p class="slab-muted">
                    ${esc(result.understanding.strategy_name)} \xB7
                    <strong>${esc(result.symbol)}</strong> \xB7
                    ${esc(result.data.timeframe_label)} \xB7
                    ${when(result.start_date)} \u2192 ${when(result.end_date)} \xB7
                    ${result.data.candles.toLocaleString()} candles from ${esc(result.data.source)}
                    ${result.cached ? ' \xB7 <span class="slab-chip">cached</span>' : ""}
                </p>
            </div>
        ...
        ${allNotes.length ? `<div class="slab-notes">${allNotes.map((n) => `<div><i class="fa-solid fa-circle-info"></i> ${esc(n)}</div>`).join("")}</div>` : ""}
        ...
        <div class="slab-metrics-grid">
            ...
            ${metricTile("Net Profit %", pct(m.net_profit_percent), tone)}
            ${metricTile("Total Trades", num2(m.total_trades))}
            ${metricTile("Win Rate", pct(m.win_rate))}
            ${metricTile("Winning Trades", num2(m.winning_trades))}
            ${metricTile("Losing Trades", num2(m.losing_trades))}
            ${metricTile("Average Win", money(m.average_win))}
            ${metricTile("Average Loss", money(m.average_loss))}
            ${metricTile("Largest Win", money(m.largest_win))}
            ${metricTile("Largest Loss", money(m.largest_loss))}
            ${metricTile("Average Trade", money(m.average_trade))}
            ${metricTile("Profit Factor", num2(m.profit_factor))}
            ${metricTile("Max Drawdown", pct(m.max_drawdown_percent), "neg")}
            ${metricTile("Sharpe Ratio", num2(m.sharpe_ratio))}
            ${metricTile("Sortino Ratio", num2(m.sortino_ratio))}
            ${metricTile("Calmar Ratio", num2(m.calmar_ratio))}
        </div>
    ...
    box.classList.remove("hidden");
    el2("slab-save-btn")?.addEventListener("click", () => void saveCurrentStrategy());
    el2("slab-export-btn")?.addEventListener("click", exportResultJson);
    drawEquityCurve(result.equity_curve);
    renderTradesAnalysis(trades);
    box.scrollIntoView({ behavior: "smooth", block: "start" });
  }
  function drawEquityCurve(points) {
    const canvas = el2("slab-equity-canvas");
    ...
    const x = (i) => padding.left + i / Math.max(1, points.length - 1) * plotW;
    const y = (v) => padding.top + plotH - (v - min) / span * plotH;
  ...
  var analysisTrades = [];
  var activeAnalysisTab = "distribution";
  ...
  function statTile(label, primary, secondary = "", tone = "", hint = "") {
  ...
  function renderTradesAnalysis(trades) {
    analysisTrades = trades || [];
    const head = document.querySelector(".slab-analysis-head");
    head?.querySelectorAll(".slab-tab").forEach((button) => {
  ...
  function switchAnalysisTab(tab) {
    ...
    document.querySelectorAll(".slab-analysis-head .slab-tab").forEach((button) => {
      button.classList.toggle("active", button.getAttribute("data-tab") === tab);
    });
    for (const name of ["distribution", "streaks", "time"]) {
      el2("slab-tab-" + name)?.classList.toggle("hidden", name !== tab);
    }
    if (tab === "distribution") {
  ...
  function renderDistributionPanel() {
    const panel = el2("slab-tab-distribution");
    if (!panel) return;
    ...
    const share = (count) => total ? (count / total * 100).toFixed(2) + "%" : "\u2014";
    panel.innerHTML = `
        <div class="slab-astat-row">
            ${statTile(
      "Expectancy",
      money(expectancy),
      expectancyPct === null ? "" : pct(expectancyPct),
      ...
      "Outliers P&L",
      money(outlierPnl),
      outliers.length ? pct(outlierPct) : "",
      outlierPnl > 0 ? "pos" : outlierPnl < 0 ? "neg" : "",
      outliers.length ? outliers.length + " trade(s) beyond 1.5x the interquartile range contributed this much." : "No statistical outliers in this run."
    )}
  ...
  function drawReturnsHistogram() {
    ...
    const xOf = (value) => pad.left + (value - min) / spread * plotW;
  ...
  function buildStreaks() {
    ...
    for (const trade of analysisTrades) {
      const kind = trade.pnl > 0 ? "win" : trade.pnl < 0 ? "loss" : "flat";
  ...
  function renderStreaksPanel() {
    const panel = el2("slab-tab-streaks");
    if (!panel) return;
    ...
    const currentLabel = current.length + " " + (current.kind === "win" ? "win" : current.kind === "loss" ? "loss" : "breakeven") + (current.length === 1 ? "" : "s");
    let worstRun = 0;
    let running = 0;
    for (const trade of analysisTrades) {
    ...
    panel.innerHTML = `
      ...
      current.kind === "win" ? "pos" : current.kind === "loss" ? "neg" : ""
            ...
            ${statTile("Average win streak", winStreaks.length ? mean(winStreaks).toFixed(2) : "\u2014")}
            ${statTile("Average loss streak", lossStreaks.length ? mean(lossStreaks).toFixed(2) : "\u2014")}
            ${statTile(
      "Worst losing run",
      money(worstRun),
      "",
  ...
  function renderTimePanel() {
    const panel = el2("slab-tab-time");
    if (!panel) return;
    if (!analysisTrades.length) {
      panel.innerHTML = emptyPanel("No trades to analyse.");
      return;
    }
    const durations = analysisTrades.filter((t) => t.exit_time).map((t) => new Date(t.exit_time).getTime() - new Date(t.entry_time).getTime()).filter((d) => Number.isFinite(d) && d > 0);
    ...
    panel.innerHTML = `
        <div class="slab-astat-row">
            ...
            ${statTile(
      "Best entry day",
      byDay.counts[bestDay] ? WEEKDAYS[bestDay] : "\u2014",
      byDay.counts[bestDay] ? money(byDay.totals[bestDay]) : "",
      "pos"
    )}
            ${statTile(
      "Worst entry day",
      byDay.counts[worstDay] ? WEEKDAYS[worstDay] : "\u2014",
      byDay.counts[worstDay] ? money(byDay.totals[worstDay]) : "",
      "neg"
    )}
        </div>
        <div class="slab-analysis-grid">
  ...
  function drawSignedBars(canvasId, labels, values, counts) {
    ...
    ctx.fillStyle = colors.dim;
    ctx.textAlign = "right";
    for (const [value, y] of [[scale, pad.top], [0, mid], [-scale, pad.top + plotH]]) {
  ...
  async function loadStrategyLabHistory() {
    const box = el2("slab-history-list");
    if (!box) return;
    try {
      const res = await apiClient.get("/api/strategy-lab/backtests?limit=20");
      ...
      box.innerHTML = rows.map((row) => {
        ...
        return `<button type="button" class="slab-history-row" data-id="${row.id}">
                <div class="slab-history-main">
                    ...
                    <span class="slab-muted">${esc(row.symbol)} \xB7 ${esc(row.timeframe)} \xB7 ${when(row.created_at)}</span>
                </div>
                <div class="slab-history-stats">
                    <span class="${tone}">${pct(profit)}</span>
                    <span class="slab-muted">${num2(row.metrics?.total_trades)} trades</span>
                </div>
            </button>`;
      }).join("");
      box.querySelectorAll(".slab-history-row").forEach((button) => {
        button.addEventListener("click", () => void openBacktest(Number(button.getAttribute("data-id"))));
      });
    } catch (err) {
      box.innerHTML = `<div class="slab-muted slab-center">${esc(errorText3(err))}</div>`;
    }
  }
  async function openBacktest(backtestId) {
    if (!backtestId) return;
    try {
      const res = await apiClient.get(`/api/strategy-lab/backtests/${backtestId}`);
  ...
  function initStrategyLabListeners() {
    el2("slab-generate-btn")?.addEventListener("click", () => void generateStrategy());
    el2("slab-market")?.addEventListener("change", onStrategyLabMarketChange);
    el2("slab-period")?.addEventListener("change", onStrategyLabPeriodChange);
    el2("slab-refresh-history")?.addEventListener("click", () => void loadStrategyLabHistory());
    el2("slab-asset")?.addEventListener("input", (event) => onStrategyLabAssetInput(event.target.value));
    el2("slab-description")?.addEventListener("keydown", (event) => {
    ...
    window.addEventListener("resize", () => {
      if (!lastResult || el2("slab-results")?.classList.contains("hidden")) return;
    ...
    safeText(el2("slab-year"), String((/* @__PURE__ */ new Date()).getFullYear()));
  }
